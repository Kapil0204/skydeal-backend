...
app.post("/debug/promote-safe-generic-offers", async (req, res) => {
  ...
  try {
    ...
    const reviewCol = db.collection("offer_review_queue");
    const rulesCol = db.collection("offer_rules");

    const candidates = await reviewCol.find({
      $or: [
        {
          title: "Domestic Flight Discount",
          rawDiscount: "FLAT ₹750 OFF*",
          flatDiscountAmount: 750
        },
        {
          title: "International Flight Discount",
          rawDiscount: "FLAT ₹2000 OFF*",
          flatDiscountAmount: 2000
        }
      ]
    ...
    for (const o of candidates) {
      ...
      if (portal !== "Goibibo") continue;
      if (!isFlightOffer(o) || isHotelOnlyOffer(o)) continue;
      if (isOfferExpired(o)) continue;

      const flat = Number(o?.flatDiscountAmount || o?.parsedFields?.flatDiscountAmount || 0);
      if (!Number.isFinite(flat) || flat <= 0) continue;

      const isDomestic = /domestic/i.test(`${o.title || ""} ${o.rawDiscount || ""} ${(o.offerCategories || []).join(" ")}`);
      const isInternational = /international/i.test(`${o.title || ""} ${o.rawDiscount || ""} ${(o.offerCategories || []).join(" ")}`);

      const rule = {
        ...o,
        offerKind: "portal",
        pricingEligible: true,
        hasDeterministicDiscount: true,
        paymentMethods: [],
        eligiblePaymentMethods: [],
        sourcePortal: "Goibibo",
        sourceMetadata: {
          ...(o.sourceMetadata || {}),
          sourcePortal: "Goibibo",
          promotedFrom: "offer_review_queue",
          promotedAt: new Date().toISOString(),
          promotionReason: "safe_generic_flat_discount_test"
        },
        promotedGenericTestRule: true,
        promotionBatch: "generic-goibibo-flat-test-001",
        offerCategories: Array.from(new Set([
          ...(Array.isArray(o.offerCategories) ? o.offerCategories : []),
          "flights",
          isDomestic ? "domestic" : null,
          isInternational ? "international" : null
        ].filter(Boolean))),
        parsedApplicablePlatforms: Array.from(new Set([
          ...(Array.isArray(o.parsedApplicablePlatforms) ? o.parsedApplicablePlatforms : []),
          "Goibibo"
        ])),
      ...
      const key = {
        promotionBatch: rule.promotionBatch,
        sourcePortal: "Goibibo",
        title: rule.title,
        rawDiscount: rule.rawDiscount,
        flatDiscountAmount: rule.flatDiscountAmount
      };

      prepared.push({ key, rule });
    }
    ...
    if (!dryRun) {
      for (const item of prepared) {
        const exists = await rulesCol.findOne(item.key);
        if (exists) {
          existing.push(item.key);
          continue;
        }

        await rulesCol.insertOne(item.rule);
    ...
    res.json({
      ...
      prepared: prepared.map((x) => ({
        key: x.key,
        title: x.rule.title,
        code: x.rule.couponCode || x.rule.code || null,
        rawDiscount: x.rule.rawDiscount,
        flatDiscountAmount: x.rule.flatDiscountAmount,
        offerKind: x.rule.offerKind,
        paymentMethods: x.rule.paymentMethods,
        offerCategories: x.rule.offerCategories,
        sourceUrl: x.rule.sourceUrl || x.rule?.sourceMetadata?.sourceUrl || null
      })),
      inserted,
      existing
    });
    ...
    res.status(500).json({
      error: e?.message || "safe generic promotion failed"
    });
  }
});
...
app.post("/debug/disable-dealpanti-pricing-rules", async (req, res) => {
  if (!requireDebugEnabled(req, res)) return;
  ...
  try {
    ...
    const query = {
      sourcePortal: "Goibibo",
      $or: [
        { sourceUrl: /dealpanti-quiz/i },
        { "sourceMetadata.sourceUrl": /dealpanti-quiz/i },
        { "sourceMetadata.sourceUrl": /dealpanti/i },
        { title: /Domestic Flight Discount/i },
        { title: /International Flight Discount/i }
      ],
      promotionBatch: "generic-goibibo-flat-test-001"
    };

    const before = await rulesCol.find(query, { projection: { _id: 0 } }).toArray();
    ...
    const update = {
      $set: {
        ...
        disabledAt: now,
        disabledReason: "GAMIFIED_QUIZ_REWARD_PAGE_REQUIRES_USER_ACTION",
        offerKind: "conditional_portal_review",
        "sourceMetadata.disabledFromPricing": true,
        "sourceMetadata.disabledAt": now,
        "sourceMetadata.disabledReason": "DealPanti quiz reward page requires user action / coupon unlock before pricing"
      },
      $addToSet: {
        reviewQueueReasons: {
          $each: [
            "GAMIFIED_QUIZ_REWARD_PAGE",
            "REWARD_DEPENDS_ON_USER_ACTION",
            "COUPON_OR_REWARD_UNLOCK_REQUIRED",
    ...
    const result = await rulesCol.updateMany(query, update);

    const after = await rulesCol.find(query, { projection: { _id: 0 } }).toArray();
    ...
    res.json({
      ...
      before: before.map((o) => ({
        portal: o.sourcePortal || o?.sourceMetadata?.sourcePortal || null,
        title: o.title || null,
        code: o.code || o.couponCode || null,
        rawDiscount: o.rawDiscount || null,
        flatDiscountAmount: o.flatDiscountAmount ?? null,
        sourceUrl: o.sourceUrl || o?.sourceMetadata?.sourceUrl || null,
        pricingEligible: o.pricingEligible ?? null,
        disabledFromPricing: o.disabledFromPricing ?? null,
        offerKind: o.offerKind || null,
        reviewQueueReasons: o.reviewQueueReasons || []
      })),
      after: after.map((o) => ({
        portal: o.sourcePortal || o?.sourceMetadata?.sourcePortal || null,
        title: o.title || null,
    ...
    res.status(500).json({
      ok: false,
      error: e?.message || "disable dealpanti pricing rules failed"
    });
  }
});
...
app.post("/debug/repair-cleartrip-audit-rows", async (req, res) => {
  if (!requireDebugEnabled(req, res)) return;
  ...
  try {
    ...
    async function updateMany(label, filter, update) {
      const before = await rulesCol.find(filter, { projection: {
        _id: 0,
        title: 1,
        code: 1,
        couponCode: 1,
      ...
      const result = await rulesCol.updateMany(filter, update);

      const after = await rulesCol.find(filter, { projection: {
        _id: 0,
        title: 1,
        code: 1,
        couponCode: 1,
      ...
      results.push({
        ...
        before: before.map((o) => ({
          ...
          paymentMethods: o.paymentMethods || o.eligiblePaymentMethods || o?.parsedFields?.paymentMethods || null,
          sourcePortal: o.sourcePortal || o?.sourceMetadata?.sourcePortal || null
        })),
        after: after.map((o) => ({
          title: o.title || null,
          code: o.code || o.couponCode || null,
    ...
    // 1) AU Cleartrip bank alias repair
    await updateMany(
      "AUCC bank canonical repair",
      {
      ...
      {
        $set: {
          repairCleartripAuditAt: now,
          "sourceMetadata.repairCleartripAuditAt": now,
          "sourceMetadata.repairReason": "Cleartrip audit: normalize AU Bank to AU Small Finance Bank for payment matching"
        },
        $addToSet: {
          paymentMethods: {
            type: "credit_card",
    ...
    // 2) Bajaj Cleartrip flight classification repair
    await updateMany(
      "CTBFL flight classification repair",
      {
        sourcePortal: "Cleartrip",
        $or: [
          { code: "CTBFL" },
      ...
      },
      {
        $set: {
          repairCleartripAuditAt: now,
          "sourceMetadata.repairCleartripAuditAt": now,
          "sourceMetadata.repairReason": "Cleartrip audit: CTBFL is a flight payment offer and should be classified as flight"
        },
        $addToSet: {
          offerCategories: "flight"
        }
    ...
    // 3) IDFC Cleartrip domestic scope repair
    await updateMany(
      "IDFCEMI domestic scope repair",
      {
        sourcePortal: "Cleartrip",
        $or: [
          { code: "IDFCEMI" },
      ...
      },
      {
        $set: {
          repairCleartripAuditAt: now,
          offerCategories: ["flight", "domestic"],
          "parsedFields.offerCategories": ["flight", "domestic"],
          "sourceMetadata.repairCleartripAuditAt": now,
          "sourceMetadata.repairReason": "Cleartrip audit: IDFCEMI 10 percent / 1200 cap row is domestic"
        }
      }
    );

    // 4) IDFC Cleartrip international scope repair
    await updateMany(
      "IDFCEMI international scope repair",
      {
        sourcePortal: "Cleartrip",
        $or: [
          { code: "IDFCEMI" },
      ...
      },
      {
        $set: {
          repairCleartripAuditAt: now,
          offerCategories: ["flight", "international"],
          "parsedFields.offerCategories": ["flight", "international"],
          "sourceMetadata.repairCleartripAuditAt": now,
          "sourceMetadata.repairReason": "Cleartrip audit: IDFCEMI 8 percent / 7500 cap row is international"
        }
      }
    );
    ...
    res.status(500).json({
      ok: false,
      error: e?.message || "repair cleartrip audit rows failed",
      stack: e?.stack || null
    });
  }
});
...
app.post("/debug/disable-expired-mmt-pricing-rules", async (req, res) => {
  if (!requireDebugEnabled(req, res)) return;

  try {
    const cutoff = String(req.query.cutoff || req.body?.cutoff || new Date().toISOString().slice(0, 10)).trim();

    if (!/^\d{4}-\d{2}-\d{2}$/.test(cutoff)) {
      return res.status(400).json({
        ok: false,
        error: "Invalid cutoff. Use YYYY-MM-DD, for example ?cutoff=2026-05-25"
      });
    }
      ...
      $and: [
        {
          $or: [
            { sourcePortal: "MakeMyTrip" },
            { "sourceMetadata.sourcePortal": "MakeMyTrip" }
          ]
        },
        { pricingEligible: true },
        { disabledFromPricing: { $ne: true } },
        { "validityPeriod.to": { $lt: cutoff } }
      ]
    };
    ...
    const before = await col.find(filter, { projection }).toArray();

    const result = await col.updateMany(filter, {
      $set: {
        pricingEligible: false,
        disabledFromPricing: true,
        disabledReason: "EXPIRED_VALIDITY_PERIOD",
        reviewStatus: "REVIEW_REQUIRED",
        "sourceMetadata.disabledFromPricing": true,
        "sourceMetadata.disabledReason": "Expired validityPeriod.to before cutoff",
        "sourceMetadata.expiredPricingDisabledAt": new Date().toISOString(),
        "sourceMetadata.expiredPricingCutoff": cutoff
      },
      $addToSet: {
        reviewQueueReasons: {
          $each: [
            "EXPIRED_VALIDITY_PERIOD",
            "DO_NOT_PRICE_EXPIRED_OFFER"
          ]
        }
      }
    });
    ...
    return res.status(500).json({
      ok: false,
      error: e?.message || "disable expired MMT pricing rules failed"
    });
  }
});
